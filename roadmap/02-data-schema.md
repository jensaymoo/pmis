...
| `073_org_unit_insert_scope_fix` | Правит `org_unit_insert` (013_org_unit.sql): `parent_id` обязателен и должен входить в `current_org_subtree()`. Не входит в исходный состав раздела — добавлена при верификации Фазы 5 (экран «Пользователи»): старая политика разрешала `parent_id IS NULL` любому `admin` без проверки поддерева, из-за чего созданная так организация проходила `INSERT`, но затем была навсегда невидима самому создателю — `org_unit_select` показывает только записи из `current_org_subtree()` |
