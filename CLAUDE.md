...
| UI-расширение | x-naive-ui (расширение Naive UI) |
| Таблицы/гриды | x-naive-ui |
...
- **x-naive-ui** — регистрируется **глобально** вместе с Naive UI; таблицы и гриды реализуются через `n-data-table`.
