...
// Единая точка отправки наружу: помечаем значение как отправленное ДО
// вызова apply и оборачиваем apply в try/catch — иначе сбой одного
// потребителя (например, неверный тип значения) рвал бы всю цепочку,
// включая страховочный flush по закрытию.
function emit(v) {
  ...
  try {
    ...
    console.error('[GridFilterHeader] apply failed:', e)
  }
}
...
watch(debounced, (v) => emit(v))

// Fallback-коммит при закрытии попапера: живой debounce (delay) — основной
// механизм, но если попапер закрывается раньше, чем сработает таймер
// (дискретный выбор, быстрый клик вне попапера), задебаунсенное значение
// может не успеть уйти — тогда коммитим текущий буфер по закрытию, чтобы
// выбор не терялся.
function flush() {
  emit(buffer.value)
}
